...
- **实现状态化接受门与重捕获**：NORMAL→SEEKING→PROBATION→NORMAL 的闭环恢复由同一公共策略模块（`safety/qp_fail_monitor.hpp` + `safety/reacquire_command.hpp`）驱动，行为测试覆盖了"SEEKING 不跑 QP、PROBATION 真跑 QP 且连续失败有界退出、观察期时长与输出边界"。ROS-free 单元 + 真实 OSQP 行为测试 16/16 通过。
...
### 表 2：Gazebo / TurtleBot3 闭环冒烟测试（circle）
...
bash src/linear_mpc_controller/scripts/tb3_smoke.sh     # Gazebo circle 冒烟测试
```
...
- **未做实机验证**——数字分别来自 Python 离线实验与 Gazebo 仿真（表 1 / 动图为 Python 参考核心，表 2 为 Gazebo / TurtleBot3）；徽章反映 CI 状态而非硬件跑。
...
| `docs/final_closeout_20260909.md` | TB3 冒烟测试收尾与开放问题 |
...
公开数字按来源分开归档：表 1 见 `results/ref_core_5run_baseline/benchmark_results.md`，表 2 见 `results/mpc_smoke_20260909/circle/tb3_smoke.json`（均为本仓库冒烟/离线产物）；姐妹仓库 `seal_results.json` 只描述其覆盖链。
